...
const Paywall = () => {
  ...
  const initializeRevenueCat = async () => {
    try {
      ...
      const res = await RevenueCatService.getOfferings();
      ...
      const cycle = typeof billingCycle === 'string' && billingCycle.length ? billingCycle : 'monthly';
      const target = productIdMap[cycle];

      const match = packages.find(
        (p) =>
          p?.identifier == target ||
          p?.product?.identifier == target ||
          p?.product?.productId == target
      );

      setSelectedPackage(match || packages[0]);
    } catch (error) {
